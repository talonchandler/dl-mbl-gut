...
class GutDataset(Dataset):
    ...
    def __init__(self, root_dir, data_channel_name="BF_fluor_path", 
                 mask_channel_name="label", x_split_width=1024,
        ...
        if useful_chunk_path is not None:
            with open(useful_chunk_path, "r") as f:
                self.item_keys = [list(line.strip().split(",")) for line in f.readlines()]
    ...
    def _find_useful_chunks(self, output_path: Path):
        ...
        print(f"Found {len(useful_item_keys)} useful chunks. Saving to {output_path}.")
    ...
    def __getitem__(self, idx: int):
        ...
        data = self.dataset[position_key][0][0, self.data_channel_index, z_index, :, x_start:x_end]
        mask = self.dataset[position_key][0][0, self.mask_channel_index, z_index, :, x_start:x_end]
        return data, mask
...
if __name__ == "__main__":
    ...
    dataset = GutDataset(base_path / dataset_path, useful_chunk_path=useful_chunk_path)
    dataset.info()

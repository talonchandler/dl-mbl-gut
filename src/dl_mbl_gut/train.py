...
def train_multiscale(
    ...
    loss = 1
    pbar = tqdm(enumerate(loader))
    for batch_id, (xhigh, xlow,  y) in pbar:

        # move input and target to the active device (either cpu or gpu)
        xhigh,xlow, y = xhigh.to(device), xlow.to(device), y.to(device)
        ...
        if tb_logger is not None:
            ...
            # check if we log images in this iteration
            if (step % log_image_interval == 0) and (len(xhigh.shape) <= 4):
                tb_logger.add_images(
                    tag="input", img_tensor=xhigh.to("cpu"), global_step=step
                )
                tb_logger.add_images(
                    tag="target", img_tensor=y.to("cpu"), global_step=step
                )
            ...
            elif (step % log_image_interval == 0) and (len(xhigh.shape) > 4):
                tb_logger.add_images(
                    tag="input",
                    img_tensor=np.max(x.to("cpu").numpy(), axis=-3),
                    global_step=step,
